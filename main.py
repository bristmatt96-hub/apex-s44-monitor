...
class TradingSystem:
    ...
    async def start(self, auto_execute: bool = False):
        ...
        logger.info(f"Capital: ${config.risk.starting_capital:,.0f}")
        logger.info(f"Max Single Name: {config.risk.max_single_name_pct:.0%}")
        logger.info(f"Daily Stop-Loss: {config.risk.daily_stop_loss_pct:.1%}")
...
def show_config():
    ...
    print("=" * 60)
    print(f"\nAUM: ${config.risk.starting_capital:,.0f}")
    print(f"Max Single Name: {config.risk.max_single_name_pct:.0%} (${config.risk.starting_capital * config.risk.max_single_name_pct:,.0f})")
    print(f"Max Positions: {config.risk.max_positions}")
    print(f"Daily Stop-Loss: {config.risk.daily_stop_loss_pct:.1%}")
    print(f"Weekly Stop-Loss: {config.risk.weekly_stop_loss_pct:.1%}")
    print(f"Monthly Drawdown: {config.risk.monthly_drawdown_pct:.1%}")
    print(f"Max Peak-to-Trough: {config.risk.max_peak_to_trough_pct:.1%}")
